...
app.post('/api/books/login', (req, res) => {
    try {
        ...
        res.status(500).send({code: "unregistered-"});
    }
});
...
app.post('/api/books/signup', (req,res) => {
    try {
        ...
        if(data.some(user => user.mail === req.body.mail)) {
            ...
            if(newUser.mail && newUser.password && newUser.name && newUser.surname) {
                ...
                res.status(400).send({code: "not-all-fields-completed"});
            }
        }
    } catch (error) {
...
app.post('/api/books', (req,res) => {
    try {
        ...
        if(newBook.title && newBook.author && newBook.publishDay && newBook.genre && newBook.imageUrl && newBook.price && newBook.bookCover && newBook.publisher && newBook.sold && newBook.synopsis) {
            ...
            res.status(422).send({code: "invalid-content"})
        }
    } catch(err) {
        res.sendStatus(500);
    }
...
app.post('/api/authors', (req,res) => {
    try {
        ...
        if(newAuthor.name && newAuthor.imgUrl && newAuthor.biography) {
